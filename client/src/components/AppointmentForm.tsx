...
import type { GroomingType, Appointment, AppointmentDto } from '../api'
import {
  createAppointment,
  updateAppointment,
  getGroomingTypes,
...
export default function AppointmentForm({ open, appointment, onClose, onSuccess }: AppointmentFormProps) {
  ...
  useEffect(() => {
    if (open && appointment && groomingTypes.length > 0) {
      ...
      const type = groomingTypes.find((t) => t.name === appointment.groomingType)
      if (type) {
  ...
  const loadGroomingTypes = async () => {
    ...
    try {
      ...
      toast.showToast(error.response?.data?.message || 'שגיאה בטעינת סוגי התספורת', 'error')
    } finally {
  ...
  const validate = (): boolean => {
    ...
    if (!selectedTypeId) {
      newErrors.type = 'נא לבחור סוג תספורת'
    }

    if (!appointmentDate) {
      newErrors.date = 'נא לבחור תאריך ושעה'
    } else {
      const now = dayjs()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message = error.response?.data?.message || 'שגיאה בשמירת התור'
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle>{appointment ? 'עריכת תור' : 'תור חדש'}</DialogTitle>
      <DialogContent>
          ...
          {loadingTypes ? (
            ...
            <>
              <TextField
                ...
                fullWidth
                error={!!errors.type}
                helperText={errors.type || (selectedType && `מחיר: ₪${selectedType.price.toFixed(2)}, משך: ${selectedType.durationMinutes} דקות`)}
                required
              >
                {groomingTypes.map((type) => (
                  <MenuItem key={type.id} value={type.id}>
                    {type.name} - ₪{type.price.toFixed(2)} ({type.durationMinutes} דקות)
                  </MenuItem>
                ))}
              </TextField>
              ...
              {selectedType && discountEligible && (
                <Alert severity="info">
                  יש לך {bookingCount} תורים קודמים - זכאי להנחה של 10%! מחיר מוזל: ₪{calculatedPrice.toFixed(2)}
                </Alert>
              )}
              ...
              {selectedType && (
                <Box sx={{ p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
                  ...
                  <Typography variant="body2">סוג: {selectedType.name}</Typography>
                  <Typography variant="body2">משך: {selectedType.durationMinutes} דקות</Typography>
      ...
      <DialogActions>
        ...
        <Button onClick={handleSubmit} variant="contained" disabled={loading || !selectedTypeId || !appointmentDate}>
          {loading ? <CircularProgress size={24} /> : appointment ? 'עדכן' : 'צור תור'}
        </Button>
      </DialogActions>
    </Dialog>
  )
